...
class NoCodeLayer(nn.Module):
    ...
    def forward(self, x: torch.FloatTensor) -> Tuple[torch.FloatTensor, float, torch.LongTensor]:
        ...
        embed_ind = None
        return x.permute(0, 3, 1, 2), diff, embed_ind
...
class VQVAE2(pl.LightningModule):
    def __init__(self,
        ...
        hidden = [in_channels*2**(i+1) for i in range(len(scaling_rates))]
        self.encoders = nn.ModuleList([Encoder(in_channels, hidden[0], res_channels, nb_res_layers, scaling_rates[0])])
        for i, sr in enumerate(scaling_rates[1:]):
            self.encoders.append(Encoder(hidden[i], hidden[i+1], res_channels, nb_res_layers, sr))
        print('enc done!')
        self.codebooks = nn.ModuleList()
        for i in range(nb_levels - 1):
            self.codebooks.append(NoCodeLayer(hidden_channels+embed_dim, embed_dim, nb_entries))
        self.codebooks.append(NoCodeLayer(hidden[-1], embed_dim, nb_entries))
        print('code done!')
        self.decoders = nn.ModuleList([Decoder(hidden[0], hidden[0], in_channels, res_channels, nb_res_layers, scaling_rates[0])])
        for i, sr in enumerate(scaling_rates[1:]):
            self.decoders.append(Decoder(hidden[i+1], hidden_channels, hidden[i], res_channels, nb_res_layers, sr))
        print('dec done!')
    ...
    def forward(self, x):
        ...
        for dec in self.decoders[::-1]:
            if len(decoder_outputs):
                decoder_outputs.append(dec(decoder_outputs[-1]))
            else:
                decoder_outputs.append(dec(encoder_outputs[-1]))
               
        # for i,c in enumerate(encoder_outputs):
        #     print(f'encoder image{i}:',c.shape)
        ...
        #         code_q, code_d, emb_id = codebook(torch.cat([*[decoder_outputs[-1]]*16, decoder_outputs[-1]], axis=1))
        #     else:
        ...
        #     decoder_outputs.append(decoder(torch.cat([code_q, *([code_q]*len(code_outputs))], axis=1)))
